...
範例目錄中的 GitHub 存放庫 [microsoft/terminal](https://github.com/microsoft/terminal) 提供使用 Pseudoconsole 的完整範例。
...
建立時，需要的字元大小 (的) 字元計數中。 這些維度會套用至最終 (終端) 展示視窗的顯示介面。 這些值可用來在 pseudoconsole 系統內建立記憶體中的緩衝區。
